...
fun SQLiteDatabase.queryAnimalMovementHistory(animalId: EntityId): List<AnimalMovement> {
    return rawQuery(QUERY_ANIMAL_MOVEMENT_HISTORY, arrayOf(animalId.toString())).use { cursor ->
        cursor.readAllItems(Cursor::readAnimalMovement)
    }
}

private val QUERY_ANIMAL_MOVEMENT_HISTORY =
    """SELECT
        ...
        |ORDER BY ${AnimalLocationHistoryTable.NAME}.${AnimalLocationHistoryTable.Columns.MOVEMENT_DATE} DESC""".trimMargin()

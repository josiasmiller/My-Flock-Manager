...
fun SQLiteDatabase.queryAnimalBirthDate(animalId: EntityId): LocalDate? {
    return rawQuery(QUERY_ANIMAL_BIRTH_DATE, arrayOf(animalId.toString()))
        .use { cursor ->
            cursor.readFirstItem {
                it.getOptLocalDate(AnimalTable.Columns.BIRTH_DATE)
            }
        }
}

private val QUERY_ANIMAL_BIRTH_DATE get() =
    """SELECT ${AnimalTable.Columns.BIRTH_DATE}
